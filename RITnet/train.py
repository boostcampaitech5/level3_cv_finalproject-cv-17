...
if __name__ == '__main__':
    ...
    project="boostcamp_final_project"
)   
    wandb.config = {
...
#    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  
    ...
    train = EyeDataset(filepath = Path2file,split='train',
                             transform = transform, **kwargs)
    
    valid = EyeDataset(filepath = Path2file , split='validation',
                            transform = transform, **kwargs)
    ...
    # test = EyeDataset(filepath = Path2file , split='test',
    #                         transform = transform, **kwargs)
...
#            
        ...
        wandb.log({"tain_miou": np.average(ious), "train_loss": loss.item()})
        ...
        wandb.log({"val_miou" : miou, "val_loss" : lossvalid })
