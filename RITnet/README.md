# NIA21 - Segmentation Model for Eye Tracking (RITnet)



model folder

```./NIA_segmentation/RITnet/```
...
```python train.py --model densenet --expname FINAL --bs 8 --useGPU True --dataset eye_data/```

To test the result:

```python test.py --model densenet --load best_model.pkl --bs 4 --dataset eye_data/```

Also, when you run test.py, the result files are stored in the "0try" folder.



eye_data folder structure

eye_data ------- train           ------- images(.jpg)
...
- You can check the log after learning in the next file in "logs.log"

``` 
./logs/FINAL/logs.log
```

- The learned weight is stored in the next folder(.pkl)

```
./logs/FINAL/models/*.pkl
```

- To create an .npy file for training from raw data(.xml)

``` 
xml_to_img.ipynb -> png_to_npy.ipynb
```
...
Original author [[AayushKrChaudhary](https://github.com/AayushKrChaudhary/RITnet/commits?author=AayushKrChaudhary)]

Edit Custom Training NIA data [TaehyungKim](https://github.com/kimtaehyeong)
